    ...
    for k in range(1):
        ...
        for b in range(B):
            ...
            for i in range(num_weights):
                ...
                # decide the logs_i
                nn = np.floor(b/1)
                delta_n = np.min((0.01, nn**(-0.5)))
                current_accpt = np.mean(accp_df, axis=1)[i]
                # if current_accpt > 0.35:
                #     logs[i] += delta_n
                # elif current_accpt < 0.30:
                #     logs[i] -= delta_n
                logs[i] = logs[i] + delta_n if current_accpt > 0.44 else logs[i] - delta_n

                si = np.exp(logs[i])
                wi_prime = rs.randn(1)*si + wi
        ...
        filename = "adp_gibbs_wts2_826" + "K" + str(k) + ".csv"
        accp_file = "adp_accp_826" + "K" + str(k) + ".csv"
        logs_file = "adp_logs_826" + "K" + str(k) + ".csv"
